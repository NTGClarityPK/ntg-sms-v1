...
Active school days of the week **per branch** (0 = Sunday … 6 = Saturday).
...
**RLS:** Branch isolation via `user_branches`
...
* `student_id` - School-facing roll number; **globally unique** across tenants; generated by `next_student_roll()` as **5-digit zero-padded** text (e.g. `00056`, `00971`)
...
**Note:** There is no separate `roll_number` column — `student_id` is the roll number.
