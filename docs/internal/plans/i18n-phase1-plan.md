...
> **Status: completed and superseded.** This is kept as a historical record of the Phase 1 build. Two of its decisions no longer hold: the default locale is now **`en-GB`, not `ar`**, and the supported locale set is `en-GB | en-US | ar` (bare `en` is a legacy value that normalises to `en-GB`). Language now resolves as `profiles.preferred_locale ?? tenants.default_locale ?? 'en-GB'`. For current behaviour see [sms-i18n-implementation-guide.md](../implementation/sms-i18n-implementation-guide.md) ("Decision 2" and "Phase 3").
