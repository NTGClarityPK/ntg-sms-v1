...
### Decision 2: Locale Storage and Resolution (Tenant Default + User Override)

Supported UI locales are `en-GB`, `en-US`, and `ar`. `en` is accepted as a legacy value and normalised to `en-GB`.

**System default is `en-GB`.** There is no implicit Arabic anywhere: a user, tenant, or request with no stored language always resolves to `en-GB`.

Language lives in **two persisted places plus one transport cookie**:

```text
1. tenants.default_locale        ← School-wide default (NOT NULL, default 'en-GB')
2. profiles.preferred_locale     ← Personal override (NULLABLE, NULL = inherit tenant default)
3. Cookie NEXT_LOCALE            ← What middleware / next-intl actually render from
```

**Effective locale** (the single rule that must hold everywhere):

```text
effectiveLocale = preferred_locale (if set) ?? tenant default_locale ?? 'en-GB'
```

- Backend resolves it in `backend/src/common/utils/locale.util.ts` and returns it on `GET /api/v1/auth/me` as `effectiveLocale`, alongside `preferredLocale` (may be `null`) and `tenantDefaultLocale`.
- Frontend mirrors the same rules in `frontend/src/lib/ui-locale.ts`.
- **`localStorage` is no longer used for locale.** It caused stale-language bugs across logins; the cookie is the only client-side carrier.

**Flow:**

- On login / OAuth callback / branch switch → read `effectiveLocale` from `auth/me` → `reconcileUiLocaleCookie(effectiveLocale)`. Reconcile **overwrites** a mismatched cookie rather than only writing when absent, so re-login and tenant switches cannot keep the previous school's language.
- User picks a language → `PATCH /api/v1/users/me/preferences { preferred_locale }` → reconcile cookie → invalidate `['auth','me']` → `router.refresh()`.
- User picks "Use school default" → same call with `preferred_locale: null` → they inherit the tenant default from then on, including future changes to it.
- Admin changes the school default → `PATCH /api/v1/tenants/me { defaultLocale }` → applies to every user without a personal override.
- Incognito / no cookie → middleware and `i18n/request.ts` resolve `en-GB` for the unauthenticated shell, then the cookie is reconciled to the effective locale once the user authenticates.

**Never** hardcode `'ar'` (or `'en'`/`'en-US'`) as a fallback. Import `SYSTEM_DEFAULT_LOCALE` from the locale utils on each side instead.
...
**Shipped file: `src/components/common/LanguageSwitcher.tsx`** (Mantine `Menu`, not a `Select`).

It offers four options: **Use school default**, English (UK), English (US), Arabic. The current selection is derived from `useLocale()` plus whether the user has a personal override (`user.preferredLocale` is a non-empty string), so "Use school default" stays ticked for inheriting users even when the rendered language matches one of the explicit options.

```typescript
const applyLocale = async (next: string | null) => {
  // next === null  → clear the override and inherit the tenant default
  await apiClient.patch('/api/v1/users/me/preferences', { preferred_locale: next });

  const effective = resolveEffectiveLocale(next, user?.tenantDefaultLocale);
  reconcileUiLocaleCookie(effective);          // from @/lib/ui-locale
  await queryClient.invalidateQueries({ queryKey: ['auth', 'me'] });
  router.refresh();
};
```

**Rules when touching this component:**

- Do **not** write `localStorage`; the cookie is the only client-side store.
- Always write the **effective** locale to the cookie, never the raw selection — picking "Use school default" must render the tenant's language, not the user's previous one.
- Invalidate `['auth','me']` so `preferredLocale`/`effectiveLocale` are refetched and the tick mark moves.
- If there's no Supabase session (e.g. the login screen), only set the cookie and refresh — there's no profile to persist to yet.
...
## Phase 3: Tenant Default + User Override (Shipped Schema)

> **History:** `profiles.preferred_locale` originally shipped as `VARCHAR(2) DEFAULT 'ar'`, which silently made every new account Arabic. Migration `20260730120000_tenant_default_locale.sql` replaced that with a tenant default plus a nullable personal override. Do not reintroduce a column-level language default.
...
-- Personal override: nullable, NO default (NULL = inherit tenant default)
ALTER TABLE public.profiles
  ALTER COLUMN preferred_locale DROP DEFAULT;

ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_preferred_locale_check
  CHECK (preferred_locale IS NULL OR preferred_locale IN ('en', 'en-US', 'en-GB', 'ar'));
```

The same migration backfilled legacy data: `preferred_locale = 'en'` became `'en-GB'`, and every legacy `'ar'` row was cleared to `NULL` (the old DB default was indistinguishable from a deliberate Arabic choice, so those users now inherit their school default).

---

### 3.2 Backend Resolution

All locale logic lives in `backend/src/common/utils/locale.util.ts` (`SYSTEM_DEFAULT_LOCALE`, `normalizeUiLocale`, `normalizeTenantDefaultLocale`, `resolveEffectiveLocale`, `resolveContentLanguage`), covered by `locale.util.spec.ts`. Import from there instead of writing inline fallbacks.

`GET /api/v1/auth/me` returns three fields, and `listUserBranches` carries `tenantDefaultLocale` per branch so branch switching resolves the new school's default:

```typescript
preferredLocale,      // string | null — personal override, null when inheriting
tenantDefaultLocale,  // string — current tenant's default_locale
effectiveLocale,      // string — resolveEffectiveLocale(preferredLocale, tenantDefaultLocale)
```

---

### 3.3 Frontend: Sync After Auth

Cookie reconciliation happens in three places, all using `reconcileUiLocaleCookie` from `@/lib/ui-locale`:

| Location | Trigger |
|----------|---------|
| `src/lib/auth/complete-session-routing.ts` | Email/password login and branch selection |
| `src/app/auth/callback/page.tsx` | Google OAuth callback |
| `src/hooks/useLocaleSync.ts` | Any later mismatch between cookie and `effectiveLocale` |

Reconcile is deliberately **overwrite-if-different**, not write-if-absent, and calls `router.refresh()` only when the cookie actually changed.

---

### 3.4 Preference and Tenant Endpoints

| Endpoint | Body | Notes |
|----------|------|-------|
| `PATCH /api/v1/users/me/preferences` | `{ preferred_locale: 'en-GB' \| 'en-US' \| 'ar' \| null }` | `null` clears the override so the user inherits the tenant default. The DTO uses `@ValidateIf` so `null` bypasses the `@IsIn` check. |
| `PATCH /api/v1/tenants/me` | `{ defaultLocale: 'en-GB' \| 'en-US' \| 'ar' }` | Guarded by `JwtAuthGuard` + `BranchGuard`, scoped to the caller's own tenant. Changes the school default for everyone without an override. Surfaced in **Settings → Business Information → Business Settings → Default language**. |

Content translation endpoints (branches, subjects, assessment types, events, core lookups) take an optional `language` query parameter and fall back to `SYSTEM_DEFAULT_LOCALE`, never `'ar'`. Frontend hooks pass the active locale explicitly (see `useTenantBranches` / `useBranchesByTenantId`).
...
### ❌ DON'T Hardcode a Language Fallback

**WRONG:**
```typescript
const language = query.language ?? 'ar';          // silently Arabic
preferredLocale: profile.preferred_locale ?? 'en-US';
document.cookie = `NEXT_LOCALE=${user.preferredLocale ?? 'ar'}`;
```

**CORRECT:**
```typescript
const language = resolveContentLanguage(query.language);              // backend
const effective = resolveEffectiveLocale(preferredLocale, tenantDefaultLocale);
```

Every fallback must route through `SYSTEM_DEFAULT_LOCALE` (`en-GB`) in `backend/src/common/utils/locale.util.ts` or `frontend/src/lib/ui-locale.ts`. Scattered literals are what produced the "school logs in as Arabic in incognito" bug.

---

### ❌ DON'T Store Locale in `localStorage`

The cookie is the only client-side carrier. A second client store drifts out of sync with `profiles.preferred_locale` and survives logout, so the next user on that browser inherits the previous user's language.

---

### ❌ DON'T Write the Cookie Only When Absent

Use `reconcileUiLocaleCookie(effectiveLocale)`. Write-if-absent means a stale cookie from a previous session or tenant wins over the server-resolved language.
...
- [ ] Login with user → `effectiveLocale` from `auth/me` is applied
- [ ] Change locale → `profiles.preferred_locale` updated
- [ ] Choose "Use school default" → `preferred_locale` becomes `NULL` and the UI switches to the tenant's `default_locale`
- [ ] Refresh page → locale persists (from cookie)
- [ ] Incognito, no cookie, unauthenticated → English (UK), not Arabic
- [ ] Incognito login as a user with no override, tenant default `ar` → portal is Arabic
- [ ] Incognito login as a user with no override, tenant default `en-GB` → portal is English
- [ ] Admin changes the school default → users **with** an override keep their language; users **without** one switch
- [ ] Switch to a branch in another tenant → language follows the new tenant's default (unless overridden)
- [ ] Google OAuth callback applies the same effective locale as email/password login
...
- [ ] `profiles.preferred_locale` is nullable with **no** column default
- [ ] `tenants.default_locale` is `NOT NULL DEFAULT 'en-GB'` and constrained to `en-GB | en-US | ar`
- [ ] No rows remain with `preferred_locale = 'en'` (legacy value normalises to `en-GB`)
