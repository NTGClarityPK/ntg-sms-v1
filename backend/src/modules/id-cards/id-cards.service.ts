...
export class IdCardsService {
  ...
  async enqueueGenerationJob(
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        created_by: userId,
        status: 'queued',
        payload: { ...dto, personIds },
        total_count: personIds.length,
        processed_count: 0,
      })
      .select('id')
      .single();
    throwIfDbError(error);
    return { data: { jobId: (data as { id: string }).id } };
  }

  async getGenerationJob(jobId: string, branchId: string): Promise<{
    data: {
      id: string;
      status: string;
      totalCount: number;
    ...
    const { data, error } = await supabase
      .from('id_card_generation_jobs')
      .select('id, status, total_count, processed_count, error_message, result')
      .eq('id', jobId)
      .eq('branch_id', branchId)
      .maybeSingle();
    throwIfDbError(error);
  ...
  async processGenerationJobPayload(
    jobId: string,
    branchId: string,
    userId: string,
    ...
    const supabase = this.supabaseConfig.getClient();
    let processed = 0;
    const results: IdCardDto[] = [];
    for (const personId of personIds) {
      const { data: batch } = await this.generate(
        {
    ...
    await supabase
      ...
      .update({
        ...
        result: { cardIds: results.map((c) => c.id) },
        updated_at: new Date().toISOString(),
      })
      .eq('id', jobId);
  }
