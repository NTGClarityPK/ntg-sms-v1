...
export function parentSetsOwnPasswordTemplate(input: {
  parentName: string;
  loginEmail: string;
  ...
  const subject = `Complete your parent account · ${schoolShort}`;
  ...
  const bodyHtml = `
    ...
    <p style="margin:0 0 20px;font-size:16px;line-height:1.6;color:#334155;">
      <strong>${escapeHtml(schoolShort)}</strong> has invited you to activate your NTG Alma parent account.
      Use the secure link below to choose your password and finish setup.
    </p>
      ...
      <tr>
        <td align="center" style="padding:8px 0 28px;">
          <a href="${escapeAttr(input.invitationLink)}" style="${ctaGreen}">
            Activate parent account &amp; set password
          </a>
        </td>
      </tr>
    </table>
