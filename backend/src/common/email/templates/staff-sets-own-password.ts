...
export function staffSetsOwnPasswordTemplate(input: {
  staffName: string;
  loginEmail: string;
  ...
  const subject = `Complete your staff account · ${schoolShort}`;
  ...
  const bodyHtml = `
    ...
    <h1 style="margin:0 0 16px;font-size:24px;font-weight:700;line-height:1.25;color:#0f172a;">
      Hello ${escapeHtml(input.staffName)},
    </h1>
    <p style="margin:0 0 20px;font-size:16px;line-height:1.6;color:#334155;">
      <strong>${escapeHtml(schoolShort)}</strong> has invited you to activate your NTG Alma staff account.
      Use the secure link below to choose your password and finish setup. You will sign in using your school email.
    </p>
      ...
      <tr>
        <td align="center" style="padding:8px 0 28px;">
          <a href="${escapeAttr(input.invitationLink)}" style="${ctaGreen}">
            Activate staff account &amp; set password
          </a>
        </td>
      </tr>
    </table>
