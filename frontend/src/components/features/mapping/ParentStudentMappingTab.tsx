...
export function ParentStudentMappingTab({ showTitle = false }: ParentStudentMappingTabProps) {
  ...
  return (
    <>
      {showTitle && <Title order={2}>{t('parentAssociationTitle')}</Title>}

      <Group justify="space-between" w="100%">
        <Group gap="sm">
          <Tooltip label={t('refresh')}>
            <ActionIcon
              id="parent-associations-btn-refresh"
              variant="light"
              size="lg"
        ...
        {associationsQuery.isLoading || associationsQuery.isRefetching || !associationsQuery.data ? (
          ...
          <Alert color={colors.error} title={t('failedToLoadAssociations')}>
            <Group justify="space-between" mt="sm">
              ...
              <Button
                id="parent-associations-btn-retry"
                variant="light"
          ...
          <Alert color={colors.info} title={t('noAssociationsFound')}>
            <Text size="sm">{t('noAssociationsHint')}</Text>
          </Alert>
        ) : (
