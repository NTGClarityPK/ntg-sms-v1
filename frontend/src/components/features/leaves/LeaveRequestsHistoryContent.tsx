...
import { LeaveReport } from '@/components/features/leaves/LeaveReport';
...
export type LeaveActionFilter = 'all' | 'pending_only' | 'non_pending';

export interface LeaveRequestsHistoryContentProps {
  isParent: boolean;
  isStudent: boolean;
  canEdit: boolean;
...
export function LeaveRequestsHistoryContent({
  isParent,
  isStudent,
  canEdit,
  activeSchoolDays,
...
}: LeaveRequestsHistoryContentProps) {
  const t = useTranslations('leave');
  const isStaffView = !isParent && !isStudent;
  ...
  const [actionFilter, setActionFilter] = useState<LeaveActionFilter>('all');
  ...
  const statusesForApi = useMemo((): LeaveStatus[] | undefined => {
    if (actionFilter === 'pending_only') {
      return ['pending'];
    }
    if (actionFilter === 'non_pending') {
      return ['approved', 'rejected', 'cancelled', 'absent'];
    }
    if (selectedStatuses.length > 0) {
      return selectedStatuses as LeaveStatus[];
    }
    return undefined;
  ...
  }, [isParent, isStudent, scopedStudentId, selectedStudentId]);
  ...
  return (
    <Stack gap="md">
        ...
        <Stack gap="md">
          <Group wrap="wrap" align="flex-end" gap="sm">
            <Box style={{ minWidth: 0, flex: '1 1 240px' }}>
              <DatePickerInput
                id="leave-history-filter-date-range"
                type="range"
            ...
            {showScopedStudentSelect && scopedStudentSelectData && onScopedStudentIdChange ? (
              <Box style={{ minWidth: 0, flex: '1 1 220px' }}>
                <Select
                  id="leave-history-scoped-student"
            ...
            {isStaffView ? (
              <Box style={{ minWidth: 0, flex: '1 1 220px' }}>
                <Select
                  id="leave-history-filter-student"
                  label={t('student')}
            ...
            <Box style={{ minWidth: 0, flex: '1 1 200px' }}>
              <Select
                id="leave-history-action-filter"
                label={t('historyActionFilter')}
                data={actionFilterData}
                value={actionFilter}
                onChange={(v) => setActionFilter((v as LeaveActionFilter) ?? 'all')}
              />
            </Box>
            ...
            <Button
              variant="subtle"
              id="leave-history-clear-filters"
              onClick={() => {
      ...
      <LeaveReport
        requests={requests}
        isLoading={leaveQuery.isLoading || leaveQuery.isRefetching}
        startDate={startDateStr}
        endDate={endDateStr}
      />
